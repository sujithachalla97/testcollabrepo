...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-900 font-sans">
      {/* Desktop sidebar: fixed width, full height, never scrolls */}
      <aside
          ...
          <div>
            {user?.role === "admin" && (
              <div className="mt-6 pt-4 border-t border-gray-100">
                <p className="font-semibold mb-3 text-xs uppercase tracking-wider text-gray-500">Manage Users</p>
            ...
            <div className="mt-6 pt-4 border-t border-gray-100">
              ...
              <button
                ...
                className="mt-4 w-full text-sm px-3 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 transition"
              >
                Logout
              </button>
            </div>
      ...
      {/* Mobile header + slide-in sidebar */}
      <div className="md:hidden w-full">
        <div className="flex items-center justify-between p-3 bg-white border-b border-gray-200">
          ...
          <button onClick={handleLogout} className="px-3 py-1 rounded-md bg-red-600 text-white text-sm">Logout</button>
        </div>
      ...
      {/* Right panel: header (fixed) + content (ONLY this scrolls) */}
      <div className="flex flex-col flex-1">
        ...
        <header className="flex-none bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-gray-900">Welcome, {user?.username ?? "User"}</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="px-3 py-2 rounded-md bg-red-600 text-white text-sm hover:bg-red-700">Logout</button>
          </div>
        </header>

        {/* Content: this is the ONLY scrollable area (vertical) */}
        <main className="flex-1 overflow-y-auto bg-gray-50 p-6">
          {/* Container sits flush next to the sidebar with no extra left offset */}
          <div className="max-w-7xl mx-auto">
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
              {/* The rendered page (Products) will scroll inside this right panel */}
              <Outlet />
            </div>
          </div>
        </main>
